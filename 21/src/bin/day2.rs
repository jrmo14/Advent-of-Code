...
fn part1(input: &Vec<(i32, i32)>) -> Result<i32, Box<dyn Error>> {
    let rv = input.iter().fold((0, 0), |acc, el| {
        match el.0 {
            0 => (acc.0 + el.1, acc.1),
            1 => (acc.0, acc.1 + el.1),
            _ => (acc.0, acc.1 - el.1)
...
fn part2(input: &Vec<(i32, i32)>) -> Result<i32, Box<dyn Error>> {
   let rv = input.iter().fold((0, 0, 0), |acc, el| {
        match el.0 {
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let input: Vec<_> = contents.iter().map(|l| {
